      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        <div className="lg:col-span-7 bg-white rounded-[3rem] border border-slate-100 shadow-sm overflow-hidden flex flex-col min-h-[700px]">
           ...
           <div className="p-10 space-y-10 overflow-y-auto no-scrollbar">
              ...
              <div className="relative"><input type="file" id="invoice-scan" accept="application/pdf,image/*" className="hidden" onChange={handleFileUpload} /><label htmlFor="invoice-scan" className={`w-full py-8 border-2 border-dashed rounded-[2rem] flex flex-col items-center justify-center gap-3 cursor-pointer transition-all ${pdfScan ? 'border-emerald-200 bg-emerald-50/20' : 'border-indigo-200 hover:border-indigo-400'}`}>{pdfScan ? <FileCheck className="w-10 h-10 text-emerald-500" /> : <DatabaseZap className="w-10 h-10 text-indigo-300" /><p className="text-[10px] font-black text-indigo-900 uppercase">Load Artifact Scan</p></label></div>
              <div className="space-y-3">{lineItems.map((item, idx) => (<div key={idx} className="flex gap-4 items-center"><input placeholder="Description" value={item.description} onChange={e => updateLineItem(idx, 'description', e.target.value)} className="flex-1 px-5 py-3.5 bg-slate-50 rounded-xl border border-transparent focus:border-indigo-200 font-bold text-xs" /><input placeholder="UGX" value={item.unitPrice === 0 ? '' : formatCurrency(item.unitPrice)} onChange={e => updateLineItem(idx, 'unitPrice', e.target.value)} className="w-32 px-5 py-3.5 bg-slate-50 rounded-xl border border-transparent focus:border-indigo-200 font-black text-xs text-right" />{lineItems.length > 1 && <button onClick={() => setLineItems(lineItems.filter((_, i) => i !== idx))} className="p-3 text-slate-300 hover:text-red-500"><Trash2 className="w-4 h-4" /></button>}</div>))}</div>
              <div className="pt-10 flex items-center justify-between"><div className="space-y-1"><label className="text-[9px] font-black text-slate-400 uppercase ml-1">Deadline</label><input type="date" value={dueDate} onChange={e => setDueDate(e.target.value)} className="bg-slate-50 border border-slate-200 rounded-xl px-4 py-2 text-[11px] font-black uppercase" /></div><button onClick={handleFinalize} disabled={!isFormValid || sending} className="px-16 py-5 bg-[#0F172A] text-[#FACC15] rounded-[2rem] font-black uppercase tracking-[0.2em] text-[12px] shadow-2xl flex items-center gap-3 active:scale-95 transition-all">{sending ? <Loader2 className="w-5 h-5 animate-spin" /> : <Zap className="w-5 h-5" />} Finalize</button></div>
           </div>
        </div>
      </div>
    </div>
